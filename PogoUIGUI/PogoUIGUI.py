# coding=UTF-8
PogoUIGUIVer = '2017/5/22 v1.2.6'

'''
PogoUIGUI 輔助工具

2017/5/22 v1.2.6 bug fix
2017/5/17 v1.2.5 PogoUpdae 整合修正
2017/5/11 v1.2.4 強迫 live Update
2017/3/23 v1.2.3 bug fix
2017/3/14 v1.2.2 增加 UIRA 命令
2017/3/10 v1.2.1 增加 RA 命令
2017/3/9 v1.2 配合 PogoPeplink 1.3.8
2017/1/26 v1.1.1 簡化 default 語法
...
UpdateSfolder = '/PogoPythonTemp/PogoAutoWorkTemp/PogoUIGUI/'
UpdateDfolder = ''
...
ftp = PogoFTP()
if ftp != 0:
    UpdateCount += PogoUpdate(ftp,'PogoUIGUI.py',UpdateSfolder,UpdateDfolder)        
    ftp.close()
...
class PogoUIGUI(Frame):
    ...
    def createWidgets(self):
        ...
        self.BPP8 ['text'] = 'UIStart'
        self.BPP8.grid(row=19, column=11)
        self.BPP8 ['command'] = self.UIUIStart

        self.BPP9 = Button(self)
        self.BPP9 ['text'] = 'UIRA'
        self.BPP9.grid(row=19, column=12)
        self.BPP9 ['command'] = self.UIUIRA
        ...
        self.BPPB1 ['text'] = '需先開到 stauts page, opt 1 多了 SN/UP, opt 2 多了 LANmac opt 3 全部'
        ...
        self.BPPC1 = Label(self)
        self.BPPC1 ['text'] = 'SN: '
        self.BPPC1.grid(row=22, column=0)
        
        self.BPPC2 = Entry(self)
        self.BPPC2 ['width'] = 16
        self.BPPC2.grid(row=22, column=1, columnspan=2, sticky = W)
      
        self.BPPC3 = Label(self)
        self.BPPC3 ['text'] = '命令檔/如 RAxxx.csv 請輸入xxx : '
        self.BPPC3.grid(row=22, column=3,columnspan=2)        

        self.BPPC4 = Entry(self)        
        self.BPPC4 ['width'] = 16
        self.BPPC4.grid(row=22, column=5,columnspan=3)
    
                  
        self.BPPC5 = Button(self)
        self.BPPC5 ['text'] = 'RA'
        self.BPPC5.grid(row=22, column=8)
        self.BPPC5 ['command'] = self.UIRA
    ...
    def UIBwait(self):
        self.Command1.delete(0,200)
        e1 = self.BaddonA2.get()
        e2 = self.BaddonA4.get()
        if e2 == '3':
            command = 'Bwait("' + e1 + '")'
        else:
            command = 'Bwait("' + e1 + '","' + e2 + '")'
    ...
    def UIHWCheck(self):
        ...
        e1 = self.BPPB3.get()
        if e1 == '0':
            command = 'HWmode, Mtype, HWmodel, HWver, FWver, HWmodels,SWver = HWCheck(driver)'
        if e1 == '1':
        ...
        if e1 == '3':
            command = 'HWmode, Mtype, HWmodel, HWver, FWver, HWmodels,SWver,LANmac,SN,UP = HWCheck(driver,3)'        
        self.Command1.insert(0, command)
    ...
    def UIUIStart(self):
        ...
        e3 = self.BPP6.get()
        if e2 == 'admin' and e3 == 'admin':
            command = 'HWmode, Mtype, HWmodel, HWver, FWver, HWmodels,SWver,LANmac,SN,UP = UIStart("' + e1 + '",driver)'
        else:
            command = 'HWmode, Mtype, HWmodel, HWver, FWver, HWmodels,SWver,LANmac,SN,UP = UIStart("' + e1 + '",driver,0,"' + e2 + '","' + e3 + '")'
        self.Command1.insert(0, command)
    ...
    def UIUIRA(self):
        ...
        e3 = self.BPP6.get()
        if e2 == 'admin' and e3 == 'admin':
            command = 'SN = UIRA("' + e1 + '",driver)'
        else:
            command = 'SN = UIRA("' + e1 + '",driver,"' + e2 + '","' + e3 + '")'
        self.Command1.insert(0, command)        
