...
def Bsleep(num):
    print '\n[' + str (time.time()) + '] Bsleep | ' + str(num)
    for x in range (num):
        print '-Bsleep[ ' + str(x+1) + ' / ' + str(num) + ']'        
        time.sleep(1)

def ErrorCon():
    print '[' + str (time.time()) + '] Error: Action Fail !! STOP the program now....'
    raise SystemExit
...
def Bdrop(point1,point2): #拖->放 元素, 第一個變數為元素開始位置 第二個變數為元素放開位置,成功回傳 1 , 失敗回傳 0
    ...
    info1 = '\n[' + str (time.time()) + '] Bdrop | ' + point1 + ' | ' + point2
    print info1
...
def Bdrop_offset(point1,px1,py1): #用 畫素XY位置 拖->放 元素, 第一個變數為元素開始位置 第二個變數為X軸變動數, 第三個變數為Y軸變動數,成功回傳 1 , 失敗回傳 0
    ...
    info1 = '\n[' + str (time.time()) + '] Bdrop | ' + point1 + ' | ' + str(px1) + ' | ' + str(py1)
    print info1
...
def Bwait(element,ctime = 3): # 等待某元素出現, 試3次, 間隔2秒, 成功回傳 1 , 失敗回傳 0
    info1 = '\n[' + str (time.time()) + '] Bwait | ' + element + ' | ' + str(ctime)
    print info1
...
def Bwait_text(text,ctime = 3): # 等待某字串出現, 試3次, 間隔2秒, 成功回傳 1 , 失敗回傳 0
    info1 = '\n[' + str (time.time()) + '] Bwait_text | ' + text + ' | ' + str(ctime)
    print info1
...
def Battribute(element): # 取得某元素的值, 回傳
    info1 = '\n[' + str (time.time()) + '] Battribute | ' + element
    print info1
...
def PingCheck(ip): # 2015/10/21 windows 環境 ping ip , 成功回傳 1 , 失敗回傳 0
...
def PingWait(ip,num = 10): # 2015/10/21 PingCheck 的延伸應用, 每3秒 ping ip, 30 秒內成功續執行, 否則中斷程式
    info1 = '\n[' + str (time.time()) + '] PingWait | ' + ip + ' | ' + str(num)
    print info1
...
def BclickT (element,adj='0'): # 依文字查找 元素 全部按下去 , element '要查的文字'  adj 查到後的第幾個元素 , 所有變數都要加 ''
    info1 = '\n[' + str (time.time()) + '] BclickT | ' + element + ' | adj= ' + adj
    print info1
    try:     
...
def BxxxT (mode, element, num='1',adj='0',adj2='A', txt = ''):
    ...
    info1 = '\n[' + str (time.time()) + '] ' + mode + ' | ' + element + ' | ' + txt + ' | num= ' + num + ' | adj= ' + adj + ' | adj2= ' + adj2
    print info1
...
def BselectTO (element1,element2): #  點選符合 2 字串條件的元素
    ...
    info1 = '\n[' + str (time.time()) + '] BselectT | ' + element1 + ' | ' + element2
    print info1
...
def Bxxx (mode, element, txt = ''):
    ...
    info1 = '\n[' + str (time.time()) + '] ' + mode + ' | ' + element + ' | ' + txt
    print info1
...
def Bver (SWver,sver,ever,cmd):
    ...
    info1 = '\n[' + str (time.time()) + '] Bver | ' + str(SWver) + ' | ' + str(sver) +  ' | ' + str(ever) +  ' | ' + str(cmd)
    print info1
